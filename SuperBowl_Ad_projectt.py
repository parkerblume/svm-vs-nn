...
def train(net, X_train, Y_train, X_test, lr, epoch):
    ...
    for epoch in range(epoch):
        ...
        loss = F.mse_loss(output, Y_train.float().view(-1,1)) #lstsq loss function L(theta) in notes (.view shapes Y_train to output)
        ...
        with torch.no_grad():
            ...
            test_loss = F.mse_loss(test_output, Y_test.float().view(-1, 1))
